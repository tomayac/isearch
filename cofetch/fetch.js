...
Fetch.prototype.get = function(index, queries, callback) {
	...
	var finalizeData = function(error,data) {
	...
	var evaluateSoundData = function(error,data) {
		if(error) {
			console.log('sound error: '+error);
			return [];
		}
		...
		if(data.length < 1) {
			...
			sound.fetchSound(soundQuery, false, finalizeData);
		} else {
			console.log('5.1 Sound data with geo information fetched!');
			//Get weather data for sounds
			weather.fetchWeather(data, finalizeData);
		}
	};
	
	var getSoundData = function(error,data) {
		//Be sure to have data before going on
		if(!error && data.length < 1) {
		...
		//Get audio for content object
		sound.fetchSound(soundQuery, true, evaluateSoundData);
	};
	
	var getVideoData = function(error,data) {
		//Be sure to have data before going on
		if(!error && data.length < 1) {
		...
		youtube.fetchVideo(youtubeQuery,getSoundData);
	};
	
	var getImageWeatherData = function(error,data) {
		//Be sure to have data before going on
		if(!error && data.length < 1) {
		...
		weather.fetchWeather(data,getVideoData);
	};
	
	var getImageData = function(error,data) {
		//Be sure to have data before going on
		if(!error && data.length < 1) {
		...
		flickr.fetchImage(flickrQuery,getImageWeatherData);
	};
	
	var getTextData = function(error,data) {
		...
		dbpedia.fetchText(dbpediaQuery, contentObject.Category, getImageData);
	};
	...
	modeldb.fetchModel(index, getTextData);
