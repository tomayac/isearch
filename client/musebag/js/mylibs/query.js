    ...
    function(config, results, loader) {
        ...
        var isAllowedExtension = function(fileName, type) {
            ...
            for (var i = 0; i < allowedTypes[type].length; i++) {
                if (allowedTypes[type][i].toLowerCase() == ext) {
                    return true;
                }
            }
        ...
        var submit = function(options, callback) {
            ...
            if (isValidQuery(query)) {
                ...
                var data = {
                    error: "the query seems to be a bit too short."
                };
                if (typeof callback === 'function') {
                    callback(false, data);
                }
